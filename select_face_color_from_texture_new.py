...
# color selection from texture, range for variation control
r = 244
g = 244
b = 244
...
def main():
    ...
    for uv in uvList:      
        ...
        if coordCount != coords:
            ...
            rSample = cmds.colorAtPoint(str(texture_name), o='RGB',u=uv[0], v=uv[1])[0]
            gSample = cmds.colorAtPoint(str(texture_name), o='RGB',u=uv[0], v=uv[1])[1]
            bSample = cmds.colorAtPoint(str(texture_name), o='RGB',u=uv[0], v=uv[1])[2]
            ...
            if rPoint == r and gPoint == g and bPoint == b:
...
def getUV():
    ...
    for i in listUVCount:
        while c != count:
            c += 1
            cmds.select(str(model_name)+'.map['+str(c)+']')
...
def getFaces():
    # count faces
    faceCount = cmds.polyEvaluate(str(model_name),f=True)
